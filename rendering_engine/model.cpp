...
namespace model
{
	...
	// Constructor
	LightSource::LightSource(const char* path, 
	...
	)
	{
		...
		// Light source position; all models begin in the middle, so this is the default
		glm::vec3 lightPos{ 1.0f, 1.0f, 1.0f };

		// Sets uniforms in object shader
		objectShader.use();
		...
		glUniform4f(lightAmbientLoc, ambient.x, ambient.y, ambient.z, ambient.w);

		GLuint lightDiffuseLoc = glGetUniformLocation(objectShader.ID, "light.diffuse");
		glUniform4f(lightDiffuseLoc, diffuse.x, diffuse.y, diffuse.z, diffuse.w);

		GLuint lightSpecularLoc = glGetUniformLocation(objectShader.ID, "light.specular");
		glUniform4f(lightSpecularLoc, specular.x, specular.y, specular.z, specular.w);

		GLuint viewPosLoc = glGetUniformLocation(objectShader.ID, "viewPos");
		glUniform3fv(viewPosLoc, 1, glm::value_ptr(input::cameraPos));

		// Sets uniforms in light source shader
		lightSourceShader.use();

		GLuint lightSourceAmbientStrenthLoc = glGetUniformLocation(lightSourceShader.ID, "ambientStrength");
		glUniform1f(lightSourceAmbientStrenthLoc, strength);
	}

	void LightSource::translate(GLfloat x, GLfloat y, GLfloat z, shaders::Shader objectShader)
	{
		glm::vec3 transVec = glm::vec3(x, y, z);
		model = glm::translate(model, transVec);
		...
		GLuint lightPosLoc = glGetUniformLocation(objectShader.ID, "light.position");
		glUniform3fv(lightPosLoc, 1, glm::value_ptr(transVec));
	}
